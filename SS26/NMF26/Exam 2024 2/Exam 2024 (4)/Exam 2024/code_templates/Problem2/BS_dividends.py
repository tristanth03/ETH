...
def buildABS(N, R, sigmaT, rT, deltaT, t, T):
    ...
    """
    h = 2*R / (N+1)
    sup = [rT(t,T)*h/6+0.5*(sigmaT(t,T)**2/2+deltaT(t,T)-rT(t,T))-sigmaT(t,T)**2/(2*h)]*(N-1)
    sub = [rT(t,T)*h/6-0.5*(sigmaT(t,T)**2/2+deltaT(t,T)-rT(t,T))-sigmaT(t,T)**2/(2*h)]*(N-1)
...
def FEM_theta(N, M, R, sigmaT, rT, deltaT, K, T, theta):
    ...
    for m in range(1,M+1):
        A_mat = buildABS(N,R,sigmaT,rT,deltaT,m*k,T)
        A_mat_m = buildABS(N,R,sigmaT,rT,deltaT,(m-1)*k,T)
        LHS = (M_mat+k*theta*A_mat)
        RHS = (M_mat-k*(1-theta)*A_mat_m)@u[:,m-1]
        u[:,m] = spsolve(LHS,RHS)
