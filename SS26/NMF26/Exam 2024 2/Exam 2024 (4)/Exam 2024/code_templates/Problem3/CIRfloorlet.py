...
def buildMassCIR(N, R, mu):
    return assembleMatrix(N,R,lambda r : r*0, lambda r: r*0, lambda r: r**(2*mu))


def buildACIR(N, R, alpha, beta, sigma, mu):
    return assembleMatrix(N,R,lambda r : r**(2*mu+1)* 1/2*sigma**2 , lambda r: (1/2*sigma**2*(2*mu+1)-alpha+beta*r) * r**(2*mu), lambda r: r**(2*mu+1))
