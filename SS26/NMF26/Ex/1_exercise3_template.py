...
def G(N,M):
    ...
    sub[-1] = 2.
    diag = [-2.]*(N)
    return (k/h**2)*(diags(sup,offsets=1)+diags(diag,offsets=0)+diags(sub,offsets=-1))

#### numerical scheme ####
def eulerexplicit(N, M):
    # todo 3 b)
    x = np.linspace(0,1,num=N+1) # <--- very important h = 1/N and Neumann on the end, hence x[1:] NOT h = 1/(N+1) & x[1:-1]
    u = np.zeros(shape=(N,M+1))
    u[:,0] = initial_value(x[1:])
    C = np.eye(N)+G(N,M)
...
#### error analysis ####
nb_samples = 5
l = np.arange(2,nb_samples+2)
N = 2**l# todo for 3 c)
M = 2*4**l# todo  for 3 c) and 3 d)
