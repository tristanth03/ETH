...
def FEM_theta(N, M, theta, beta):
    ...
    for m in range(1,M+1):
        tm = (m/M)**beta; tmm = ((m-1)/M)**beta
        k = tm-tmm
        LHS = (M_mat+k*theta*A_mat)
        RHS = (M_mat-k*(1-theta)*A_mat)@u[:,m-1]+k*(theta*build_F(tm,N)+(1-theta)*build_F(tmm,N))
        u[:,m] = spsolve(LHS,RHS)
    return u[:,-1]
...
def conv(u_v):
    ...
    norm1*=(1/(len(u_v[0])+1)); norm2*=(1/(len(u_v[1])+1))
    
    return np.log(np.sqrt(norm1)/np.sqrt(norm2))/np.log(2)
    


conv_rate =conv(u_v) # Estimate the convergence rate
