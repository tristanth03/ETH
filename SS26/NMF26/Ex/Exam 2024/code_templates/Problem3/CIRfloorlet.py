...
def buildMassCIR(N, R, mu):
    ...
    return assembleMatrix(N,R,lambda r: 0*r, lambda r: 0*r, lambda r: r**(2*mu))


def buildACIR(N, R, alpha, beta, sigma, mu):

    return assembleMatrix(N,R,
                          lambda r: 0.5*sigma**2 * r**(2*mu+1),
                          lambda r: (0.5*sigma**2*(2*mu+1)+beta*r-alpha)*r**(2*mu),
                          lambda r: r**(2*mu+1))


def g(s, T, T1, K):

    return s*(T1-T)*np.maximum(K-(1-s)/((T1-T)*s),0)
...
def FEM_theta(N, k, R, t, alpha, beta, sigma, mu, u_init, theta):
    ...
    u0 = np.zeros(shape=(N+1,M+1))
    u0[:,0] = u_init
    LHS = (M_mat+k*theta*A_mat)

    for m in range(1,M+1):
        RHS = (M_mat-k*(1-theta)*A_mat)@u0[:,m-1]
        u0[:,m] = spsolve(LHS,RHS)
    return u0[:,-1]
...
if __name__ == "__main__":
    ...
    u_1 = FEM_theta(N,k,R,T1-T,alpha,beta,sigma,mu,g(u_0,T,T1,K),theta)
