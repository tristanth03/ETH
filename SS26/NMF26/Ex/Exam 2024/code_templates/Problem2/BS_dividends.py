...
def buildABS(N, R, sigmaT, rT, deltaT, t, T):
    ...
    h = 2*R / (N+1)
    sup = [rT(t,T)*h/6+1/2*(sigmaT(t,T)**2/2+deltaT(t,T)-rT(t,T))-sigmaT(t,T)**2/(2*h)]*(N-1)
    sub = [rT(t,T)*h/6-1/2*(sigmaT(t,T)**2/2+deltaT(t,T)-rT(t,T))-sigmaT(t,T)**2/(2*h)]*(N-1)
    diag = [2*rT(t,T)*h/3+sigmaT(t,T)**2/(h)]*(N)
...
def FEM_theta(N, M, R, sigmaT, rT, deltaT, K, T, theta):
    ...
    u[:,0] = u_init(np.exp(np.linspace(-R,R,N+2)[1:-1]),K)
    M_mat = buildMassBS(N,R)
    for m in range(1,M+1):
        LHS = (M_mat+k*theta*buildABS(N,R,sigmaT,rT,deltaT,(m)*k,T))
        RHS = (M_mat-k*(1-theta)*buildABS(N,R,sigmaT,rT,deltaT,(m-1)*k,T))@u[:,m-1]
        u[:,m] = spsolve(LHS,RHS)
    return u[:,-1]
