...
def build_BSMatrix(N,sigma,r,R):
    h = 2*R/(N+1)
    sup = [-sigma**2 / (2*h) + (0.5*sigma**2 - r)*0.5 + r*h / 6]*(N-1)
    sub = [-sigma**2 / (2*h) - (0.5*sigma**2 - r)*0.5 + r*h / 6]*(N-1)
    diag = [sigma**2 / h  + 2*r*h / 3]*(N)
    return (sp.diags(sup,offsets=1)+sp.diags(diag,offsets=0)+sp.diags(sub,offsets=-1))
